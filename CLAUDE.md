...
- **⚠️ 保持神秘感(重要產品要求)**:對使用者**完全不透露這是 MBTI / 性格分類測驗,也不顯示型號(如 INFP)或 MBTI 暱稱(如「調停者」)**。
  - 型號 `code`、字母 `letter`、暱稱 `nickname` **僅供內部計分與資料對應,絕不渲染到畫面**。
  - 介面 / 文案 / 題目不得出現「MBTI / 16 型人格 / 性格類型 / E·I·S·N」等字眼。
  - 整體包裝成「酒吧讀出你的靈魂,為你斟上專屬的那一杯」。
- **結果頁顯示**:氛圍小標 → 靈魂調酒(注杯動畫)→ 性格頭銜 `title` → 調酒中/英文名 → 金句 `caption` → 結果文案 `desc`。不顯示型號、不顯示契合度。
