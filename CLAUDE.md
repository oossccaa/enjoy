...
- 動畫:**純 CSS**(`main.css` 的 `@keyframes` fadeUp / fadeIn / floatY / glowPulse;結果頁酒照是 `.res-img` 載入後的 opacity/scale 顯影 transition)。`GlassSvg` 的注杯機制保留,目前僅首頁示意杯使用。
...
src/
├── main.ts                 # 掛載 App
├── App.vue                 # 狀態機(home/quiz/result/share/gallery)+ #glow 環境光 + 畫面淡入淡出
...
- **外部相依**:Google Fonts 走 CDN,離線時字體會 fallback(功能不受影響)。
